# PitLore Dogfood Evidence Log

> 这是证据日志，不是营销进度。初始观察窗口 **2026-07-16 至
> 2026-07-22（Asia/Shanghai）** 已结束，且未达到当时的“连续 7 天”退出标准。
> 2026-07-27 起按 D-016，7 天观察降为非阻塞产品质量信号，不再阻塞开源、开发、
> release 或 Phase 推进。Demo fixture、协议单测、synthetic 数据和同一天重复烟测
> 仍不能冒充连续真实使用。

## 非阻塞产品质量观察目标

- 至少一个真实个人/团队仓连续使用 7 个日历日。
...
这些目标用于判断产品是否在真实工作中有用，不是开源、开发、release 或 Phase 推进
门槛。未达到时必须保持“未知/样本不足”的事实，达到后也不能把工程测试冒充为外部采用。

## 2026-07-27 计分板

| 指标                     |                          目标 |                                                                                                     实际 | 判断                                             |
| ------------------------ | ----------------------------: | -------------------------------------------------------------------------------------------------------: | ------------------------------------------------ |
| 连续真实使用             |                          7 天 |                                                   初始窗口活跃 4/7 天（7/16、17、18、22）；最长连续 3 天 | 历史窗口未达到观察目标；不阻塞开发               |
| 真实使用仓               |                            ≥1 |                                                                                        1（PitLore 自身） | 已开始，但有强自测偏差                           |
| candidate lifecycle      | 真实 candidate 均有独立人决定 | 60 条历史 approved；当前 45 条 candidate 仍未人审；29 条有 current advisory，16 条未 advisory review | 观察样本不足；advisory review 不是批准           |
...
| 2026-07-19 至 2026-07-21 | 无可验证的真实活跃记录                                                                                                 | 不计入                                                                                          | 不计入                                                                                             | 无                                                          | 因此打断连续 streak                                                                                 |
...
| 2026-07-27               | 转为持续开源开发；完成 discovery facets、008→009 reindex、安全审计、依赖修复与 clean public import                    | 多次使用同一 current catalog retrieve；13 个变更 production source 的最终 check 均为 0 findings | 三个真实回填问题只记为 private candidate；当前共 45 candidate，未 agent-approve                      | **无人工 usefulness/detector 评分，未写新 evidence record** | 43 files / 373 tests、真 PG17 升级/回填/restore 通过；仓库公开是分发事实，不冒充产品效用或社区采用   |
...
1. 遇到真实非烟测任务时继续记录；连续观察可从任一真实活跃日开始，空档必须如实保留，
   不为凑 streak 伪造使用。
...
5. 只有连续 7 天和足量 current-catalog 人工样本都成立，才能宣告该产品质量观察目标
   已满足；不得据此倒推社区采用，也不再使用“Phase 1 退出”作为开发门禁。
